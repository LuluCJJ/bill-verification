...
- 静态样例路径：使用合成票据图片和预置 AI 识别 JSON，保证端到端产品链路可演示。
- 模型测试路径：通过 OpenAI-compatible 测试脚本验证公司内部模型的纯文本和图片输入能力。

注意：当前页面上的“运行样例核验”默认使用预置票面识别结果，不会自动调用多模态模型。这样做是为了在内部模型图片输入协议未确认前，先把系统数据、票面结果、配置、比对和反馈闭环跑通。真实模型提取会作为下一步接入到上传文档链路。

模型配置可以在 Demo 页面里的“模型配置与测试”区域填写。配置会保存到本地 `config.local.json`，该文件不会提交到 Git。火山方舟 OpenAI-compatible 地址和豆包多模态模型名已作为页面默认值，也可以参考 `.env.example`。

推荐技术栈：

- 前端：React + Vite + TypeScript。
- 后端：Python FastAPI。
- 环境：项目内 `.venv`，通过 `requirements.txt` 和 `start_demo.bat` 管理后端依赖。
- 存储：本地文件或 SQLite。
- 模型：公司内部多模态大模型接口，暂按 Qwen3.5 35B A3B 类接口设计。

## 本地环境

Windows 环境可参考以下方式启动后端 Demo：
...
当前仓库处于方案设计和 Demo 实现阶段。当前优先实现完整轻量闭环：合成样例、系统 JSON、配置管理、静态识别结果、规则比对、反馈记录和模型接口测试脚本。
