...
- 业务先按模板配置是否启用 AI、要检查哪些字段、字段来源、票面别名和给 AI 的输入。
- AI 只在当前模板已发布字段集合内做定向提取，不做开放式票据字段发现。
- 系统展示 AI 识别值、系统值、差异说明、风险等级和票面证据。
- 日期转换、金额大小写转换、账号归一等最终比对规则后续由交易系统/MAP 函数包负责，Demo 中仅做效果预览。
- 权签人基于 AI 提示完成最终人工确认。
- 人工反馈沉淀为模板字段配置、别名、给 AI 的输入和模型优化样本。

当前 AI 输入和输出进一步收窄：

- 输入：票据图片 + 当前模板 + 已配置字段清单 + 每个字段的系统来源、票面别名和给 AI 的输入。
...
当前后端会把模板别名同时用于 prompt 和模型返回后的确定性补映射，因此业务修改某个模板字段的“票面可能叫法”后，可以影响模型理解，也可以影响后端兜底映射。

## 当前 Demo

当前 Demo 已实现四个产品页面：

- **付款核验**：先选择核验模板，页面展示本模板待检查的系统结构化字段，再查看系统支付指令和票面文档；点击“开始 AI 预审”后，真实调用多模态模型逐项提取模板字段，未提取到的字段会进入风险提示；当前包含“入账行别名漏识别”的反馈闭环用例。
- **模板调优**：支持模板新增、复制、删除和字段增删改；按模板维护 AI 是否启用、发布状态、待提取字段、系统来源、票面别名和给 AI 的输入。
