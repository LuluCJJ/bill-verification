...
def s1():
    ...
    rows = [
        ...
        ["模板运营机制", "模板新增/复制、字段增删改、票面别名、AI 识别说明、反馈样本", "让字段叫法和模板差异通过配置持续迭代"],
    ]
...
def s3():
    ...
    rows = [
        ["层级", "输入", "输出", "控制方式", "关键风险"],
        ["1. 模板字段配置", "银行/国家/单据模板 + MAP 字段", "待检查字段、系统来源、票面别名、AI 识别说明", "只有发布模板才启动 AI", "未配置模板无法进入预审"],
...
def s4():
    ...
    rows = [
        ...
        ["2. 字段配置", "模板字段、系统来源、票面别名、AI 识别说明", "发布模板字段配置", "可调用 AI 的字段清单", "业务配置 + MAP 管控"],
...
def s6():
    ...
    rows = [
        ...
        ["AI 识别说明", "合并说明字段含义、常见位置和提取注意事项", "不直接做规则匹配，只作为模型理解提示", "避免自然语言描述变成泛化规则"],
        ["反馈优化", "下次调用模型时带入新叫法或更清晰的提取说明", "保存为模板别名或 AI 识别说明建议，支持回滚", "按模板版本发布"],
    ]
    table(d, 60, 220, [190, 420, 430, 410], [58, 110, 110, 110], rows)
...
def s7():
    ...
    rows = [
        ...
        ["模板调优", "业务配置人员", "新增/复制模板，增删改字段，维护票面别名和 AI 识别说明", "少技术术语，按模板和字段组织", "已支持基础管理"],
