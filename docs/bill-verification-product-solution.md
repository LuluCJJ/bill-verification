...
5. AI 产品完成票面原始 Key/Value 提取，并结合规则资产完成字段映射和风险比对。
...
- AI 产品：负责票面理解、原始 Key/Value 提取、候选字段映射、证据定位和置信度输出。
...
在此基础上，再输出 `extracted_fields`，表示该原始字段映射到 MAP/AP 哪个标准字段。

### 5.2 字段归一

字段映射是本方案最关键的产品能力。它负责把不同票面表述映射到 MAP/AP 的标准字段。建议不要完全交给提示词自由发挥，而是采用“模型候选 + 规则配置 + 模板上下文”的组合方式。

建议采用三类归一方式：

1. 规则别名映射  
   适用于有明确 key 的场景，例如“入账行”明确配置为收款方银行别名。匹配方式应可配置为精确、包含或模糊。
