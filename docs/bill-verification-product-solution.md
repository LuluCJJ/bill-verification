...
本方案的最新定位是：建设一个“基于模板字段配置”的票据一致性 AI 预审能力。业务先在模板上配置本次要核验的字段集合、系统来源、票面别名和给 AI 的输入；AI 只在该明确范围内做定向提取；最终比对规则由交易系统/MAP 函数包实施和预览，权签人仍负责最终判断。
...
- 支持业务按模板配置要提取的字段、系统来源、票面别名和给 AI 的输入。
...
3. MAP/AP 根据银行、国家、付款方式或业务选择确定票据模板，必要时允许人工修正模板。
4. 权签人进入核验页后，先看到当前模板以及该模板本次待检查的系统结构化字段。
5. 作业人员扫描或上传纸面付款文件，页面并列展示系统支付指令和票面文档。
6. MAP/AP 根据当前模板判断是否存在已发布 AI 字段配置。
7. 若模板未配置、未启用或未发布，AI 不启动；若已配置，则将票据文件、系统指令和字段配置传递给 AI 产品。
8. AI 产品对每一个模板字段返回提取值、原文证据和置信度；未提取到或异常的字段直接进入风险提示。
9. 交易系统/MAP 调用函数包完成日期、金额、账号、名称等比对规则。
10. MAP/AP 权签页面默认展示风险项，通过项可展开查看。
11. 权签人作出人工判断：确认一致、确认不一致、AI 识别错误、忽略提示或提交优化。
12. 系统沉淀人工反馈，进入后续模板、别名、规则和模型优化。
...
- 模板配置资产：维护字段清单、系统来源、票面别名、给 AI 的输入和反馈样本。
...
- `document_items`：票面原始 Key/Value，例如 `入账行：中国工商银行上海分行`。当前建议同时保留 `mapped_field`、`mapped_display_name`、`source_system_field` 和 `mapping_source`，让业务能看出这条原文最终对应哪个系统结构化字段。
- 原文证据：页码、区域、原文片段。
- 不确定项也应保留原始 Key/Value，不要强行映射。

在此基础上，再输出 `extracted_fields`，表示该字段属于当前模板配置中的哪一个 `field_id`。未配置字段不应进入正式提取结果。

如果模型只返回了 `document_items`，但没有稳定返回 `extracted_fields`，后端应根据当前模板字段别名做确定性补映射，补出 `mapped_field` 和对应 `extracted_fields`，再进入后续核验。
...
- 进入 Prompt：告诉模型当前模板下票面可能出现的叫法，提升模型看图和理解字段的稳定性。
- 进入后端规则：模型返回后，用模板别名对 `document_items.raw_key` 做确定性补映射，避免完全依赖模型自己输出 `normalized_field`。

业务配置的“给 AI 的输入”主要帮助模型理解，不应直接变成宽泛匹配规则；真正参与确定性映射的应是边界清楚的字段别名。
...
- 新增模板：用于新银行、新国家或新付款文件样式。
- 复制模板：复用已有银行或相近国家模板，减少重复配置。
- 停用/删除模板：避免未验证模板进入正式核验。
- 字段增删改：业务可按模板维护本次要检查的系统字段集合。
...
- `ai`：模型直接给出字段映射。
- `ai_uncertain`：模型给出低置信度候选，需要人工关注。
- `template_alias_rule`：后端根据当前模板字段别名补映射。
- `manual_feedback`：来自人工反馈沉淀的配置。
