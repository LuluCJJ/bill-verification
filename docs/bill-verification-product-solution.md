...
本方案的最新定位是：建设一个“基于模板字段配置”的票据一致性 AI 预审能力。业务先在模板上配置本次要核验的字段集合、系统来源、票面别名和 AI 识别说明；AI 只在该明确范围内做定向提取；最终比对规则由交易系统/MAP 函数包实施和预览，权签人仍负责最终判断。
...
- 支持业务按模板配置要提取的字段、系统来源、票面别名和 AI 识别说明。
...
- 模板配置资产：维护字段清单、系统来源、票面别名、AI 识别说明和反馈样本。
...
业务配置的“AI 识别说明”主要帮助模型理解，不应直接变成宽泛匹配规则；真正参与确定性映射的应是边界清楚的字段别名。
